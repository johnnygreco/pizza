...
describe("derivePiRangeFromDependency", () => {
  it("derives a tilde range from the Pi dependency version", () => {
    expect(derivePiRangeFromDependency("0.66.1")).toBe("~0.66.0");
  });
});

describe("describePiRange", () => {
  it("renders a user-facing x-range for tilde requirements", () => {
    expect(describePiRange("~0.66.0")).toBe("0.66.x");
  });
});

describe("isPiVersionCompatible", () => {
  it("accepts versions within the same supported major/minor", () => {
    expect(isPiVersionCompatible("0.66.5", "~0.66.0")).toBe(true);
  });

  it("rejects the next Pi minor release", () => {
    expect(isPiVersionCompatible("0.67.0", "~0.66.0")).toBe(false);
  });

  it("uses the package-declared support range by default", () => {
    expect(isPiVersionCompatible("0.66.1", SUPPORTED_PI_RANGE)).toBe(true);
  });
});

describe("compatibility messaging", () => {
  it("summarizes compatible Pi versions", () => {
    expect(getPiCompatibilitySummary("0.66.5")).toBe("v0.66.5 (compatible with 0.66.x)");
  });

  it("summarizes incompatible Pi versions", () => {
    expect(getPiCompatibilitySummary("0.67.0")).toBe("v0.67.0 (expected 0.66.x)");
  });

  it("returns no warning for compatible Pi versions", () => {
    expect(getPiCompatibilityWarning("0.66.5")).toBeNull();
  });

  it("returns a warning for incompatible Pi versions", () => {
    expect(getPiCompatibilityWarning("0.67.0")).toContain("expects Pi 0.66.x");
    expect(getPiCompatibilityWarning("0.67.0")).toContain("current Pi is v0.67.0");
  });
});
