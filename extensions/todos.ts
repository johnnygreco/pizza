...
function normalizeTodoSettings(raw: Partial<TodoSettings>): TodoSettings {
	...
	const gcDays = (Number.isFinite(raw.gcDays) ? raw.gcDays : DEFAULT_TODO_SETTINGS.gcDays) as number;
	return {
		gc: Boolean(gc),
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme) {
			...
			const todoDetails = details as Extract<TodoToolDetails, { todo: TodoRecord }>;
